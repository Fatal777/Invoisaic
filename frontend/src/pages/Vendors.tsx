...
export default function Vendors() {
  ...
  const getStatusConfig = (status: string) => {
    const configs = {
      ...
      flagged: { color: 'bg-red-500/20 text-red-400', icon: AlertTriangle },
    };
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-8">
          <Card 
            className={`bg-white/5 backdrop-blur-xl border transition-all cursor-pointer ${
              filterStatus === 'all' ? 'border-red-500' : 'border-white/10 hover:border-white/20'
            }`}
          ...
          <Card 
            className={`bg-white/5 backdrop-blur-xl border transition-all cursor-pointer ${
              filterStatus === 'flagged' ? 'border-red-500' : 'border-white/10 hover:border-white/20'
            }`}
          ...
          <Card className="bg-white/5 backdrop-blur-xl border-white/10">
            <CardContent className="p-4">
              <div className={`text-2xl font-bold mb-1 ${
                stats.avgRisk < 20 ? 'text-green-400' : stats.avgRisk < 50 ? 'text-yellow-400' : 'text-red-400'
              }`}>
                {stats.avgRisk}%
              </div>
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-red-500"
                />
              </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`text-xs font-semibold ${
                          ...
                          vendor.riskScore < 50 ? 'text-yellow-400' : 'text-red-400'
                        }`}>
                          {vendor.riskScore}%
                        </span>
                      </div>
                      <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
                        <div 
                          className={`h-full transition-all ${
                            ...
                            vendor.riskScore < 50 ? 'bg-yellow-500' : 'bg-red-500'
                          }`}
