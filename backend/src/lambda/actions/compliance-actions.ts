 ...
 */
async function validateTaxCompliance(parameters: any[]): Promise<any> {
  ...
  if (rateMatch) {
    complianceChecks.push({
      ...
      regulation: kbResults.regulations?.find((r: any) => r.includes('tax rate'))
    });
  } else {
    complianceChecks.push({
      check: 'tax_rate_validation',
 ...
 */
async function screenRegulatoryRequirements(parameters: any[]): Promise<any> {
  const invoiceData = getParameter(parameters, 'invoice_data');
  const jurisdiction = getParameter(parameters, 'jurisdiction') || 'US';
 ...
 */
async function queryComplianceKnowledgeBase(parameters: any[]): Promise<any> {
 ...
 */
async function verifyVendorQualifications(parameters: any[]): Promise<any> {
  ...
  const requiredChecks = qualificationChecks.filter(c => 
...
function getExpectedTaxRates(jurisdiction: string): number[] {
...
function getMandatoryFields(jurisdiction: string): string[] {
  const baseFields = [
    'invoice_number',
    'invoice_date',
    'vendor_name',
  ...
  const jurisdictionFields: Record<string, string[]> = {
    'US': ['tax_id'],
...
function getRetentionPeriod(jurisdiction: string): number {
...
function getRequiredApprovalLevel(amount: number): string {
  if (amount >= 100000) return 'CEO + Board';
  if (amount >= 25000) return 'VP + CFO';
  if (amount >= 5000) return 'Senior Manager + Finance';
  if (amount >= 1000) return 'Department Manager';
